...
class Klevu_Content_Model_Content extends Klevu_Search_Model_Product_Sync
{
    ...
    public function syncCmsData($store)
    {
            ...
            $actions = array(
                    'delete' => $this->getConnection()
                        ...
                        ->where(
                            "((k.store_id = :store_id AND v.store_id != 0) AND (k.type = :type) AND (k.product_id NOT IN ?)) OR ( (k.product_id IN ('".$eids."') OR (c.page_id IS NULL) OR (c.is_active = 0)) AND (k.type = :type) AND k.store_id = :store_id)",
                            $this->getConnection()
                                ->select()
                                ->from(
    ...
    protected function addcmsData(&$pages)
    {
        ...
        $data = Mage::getModel('cms/page')->getCollection()->addFieldToSelect("*")->addFieldToFilter(
            'page_id', array(
            'in' => $page_ids
            )
        );
